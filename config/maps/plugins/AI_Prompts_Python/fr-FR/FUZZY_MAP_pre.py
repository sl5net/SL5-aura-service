...
# fichier config/maps/plugins/ /FUZZY_MAP_pr.py
...
FUZZY_MAP_pre = [
    ...
    (multiline_string_1, r'^(ai|a|œuf|payer|Hé|Salut|h)\s*\w?\ş*rapide\s*(pour|avant|ver)?(\s+(Python|fois|à|titane))?.*$'),


    # Invite Python


    # Invite Python Brighton

    # EXAMPLE: Invite Python

    (multiline_string_1, r'^(Python|fois|à|titane|depuis|à chaîne|Brighton)\s*rapide.*$'),

    # EXAMPLE: Instructions système

    (System_Instructions_20260721_1417, r'^(système instruire\w*tion\w*)$', 10, {
        'cache': False,
    ...
    (multiline_string_1, r'^(Breitenbrunn)\s*rapide.*$'),


    # EXAMPLE: Programme Python

    ('Python prog', r'\bTimes programme'),
...
test = """
    ...
    # Formulation rapide pour les balises EXEMPLE (allemand, pour les IA)
...
Kurzvorschlag: Neues EXAMPLE-Format (TTS-freundlich, sofort matchend, ohne Sonderzeichen)
    ...
    # Formulation rapide pour les balises EXEMPLE (anglais, pour les IA)
...
Das Zielformat ist:
# EXAMPLE: b regex_group b

('Interessanter Text', r'\b(Expression régulière_groupe)\b')
...
# EXAMPLE: b Esprits paradigmatiques

Output: ('pragmatic minds GmbH 2019', r'\b(paradigme Esprits|pragmatique esprits)\b', 75, # min_accuracy
 {'command_flags': re.IGNORECASE})
