...
# config/maps/plugins/AI_Prompts_Python/es-DE/FUZZY_MAP_pre.py

# archivo de configuración/maps/plugins/ /FUZZY_MAP_pr.py
...
FUZZY_MAP_pre = [
    ...
    (multiline_string_1, r'^(ai|a|huevo|pagar|ey|Hola|h)\s*\w?\ş*inmediato\s*(para|antes|ver)?(\s+(Python|veces|en|titanio))?.*$'),


    # mensaje de Python


    # Mensaje de Python Brighton

    # EXAMPLE: mensaje de Python

    (multiline_string_1, r'^(Python|veces|en|titanio|de|en cadena|Brighton)\s*inmediato.*$'),

    # EXAMPLE: Instrucciones del sistema

    (System_Instructions_20260721_1417, r'^(sistema instruir\w*ción\w*)$', 10, {
        'cache': False,
    ...
    (multiline_string_1, r'^(Breitenbrunn)\s*inmediato.*$'),


    # EXAMPLE: Programa Python

    ('Python prog', r'\bveces programa'),
...
test = """
    ...
    # Formulación rápida para etiquetas EJEMPLO (alemán, para IA)
...
Kurzvorschlag: Neues EXAMPLE-Format (TTS-freundlich, sofort matchend, ohne Sonderzeichen)
    ...
    # Texto rápido para etiquetas EJEMPLO (inglés, para IA)
...
Das Zielformat ist:
# EXAMPLE: b grupo_regex b

('Interessanter Text', r'\b(expresión regular_grupo)\b')
...
# EXAMPLE: b Mentes paradigmáticas

Output: ('pragmatic minds GmbH 2019', r'\b(paradigma Mentes|pragmático mentes)\b', 75, # min_accuracy
 {'command_flags': re.IGNORECASE})
