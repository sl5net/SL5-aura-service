...
FUZZY_MAP_pre = [
    ...
    (multiline_string_1, r'^(ai|a|egg|pay|hey|hi|h)\s*\w?\ş*prompt\s*(for|before|ver)?(\s+(Python|times|at|titanium))?.*$'),
    ...
    (multiline_string_1, r'^(Python|times|at|titanium|from|at chain|Brighton)\s*prompt.*$'),

    # EXAMPLE: System instructions

    (System_Instructions_20260721_1417, r'^(system instruct\w*tion\w*)$', 10, {
        'cache': False,
...
test = """
    ...
    # Prompt formulation for EXAMPLE tags (German, for AIs)
...
Kurzvorschlag: Neues EXAMPLE-Format (TTS-freundlich, sofort matchend, ohne Sonderzeichen)
    ...
    # Prompt wording for EXAMPLE tags (English, for AIs)
...
('Interessanter Text', r'\b(Regex_group)\b')
...
# EXAMPLE: b Paradigm Minds

Output: ('pragmatic minds GmbH 2019', r'\b(paradigm Minds|pragmatic minds)\b', 75, # min_accuracy
 {'command_flags': re.IGNORECASE})
