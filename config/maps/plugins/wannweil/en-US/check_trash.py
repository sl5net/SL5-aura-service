...
# Find the .env file exactly in this folder
...
# List for German weekdays (0 = Monday, 6 = Sunday)
                    ...
                    if best_day:
                        ...
                        if 2 < rel_x < 50:
                            ...
                            try:
                                ...
                                # The incorrect tag_name lines have been removed here
...
def save_to_csv(termine, script_path, pdf_path):
    ...
    try:
        with open(csv_path, 'w', encoding='utf-8') as f:
            ...
            for t in termine:
                # Here we get the German name via the index (datum.weekday())
...
def check_and_notify(force_test=False):
    ...
    if force_test:
        ...
        if zukunft:
            ...
            # msg = f"Next pickup: {n['date'].strftime('%d.%m.%Y')} ({' & '.join(n['names'])})"
...
def check_csv_alerts():
    ...
    with (open(csv_file, 'r', encoding='utf-8') as f):
        ...
        for row in reader:
            ...
            try:
                # Use datetime.datetime.strptime here


                now = datetime.datetime.now()


                # 1. Read data & check wildcards
                ...
                # end = datetime.datetime.max # "valid forever"
                ...
                # msg = row.get('Message', '').replace('"', '\\"') # Escape quotes


                # Definition: What is considered an “empty” date? (wild card)
                ...
                if (is_start_always and is_end_forever ) or (is_start_always and now <= end + tolerance) or (start - tolerance <= now and is_end_forever) or (start - tolerance <= now <= end + tolerance) :
                    ...
                    if 'V' in modes:
                        # Here we only clean up the voice output (emojis gone)
...
if __name__ == "__main__":
    ...
    # True on weekdays, False on Saturday and Sunday
