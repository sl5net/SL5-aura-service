...
W systemach zgodnych z POSIX (Linux i macOS) ścieżki bezwzględne pasujące do katalogu domowego użytkownika (np. `/home/nazwa użytkownika/`) są konwertowane przy uruchomieniu na ścieżki względne `~`. Dzięki temu długość ciągów jest krótsza i sprawia, że wygenerowane reguły można przenosić pomiędzy różnymi użytkownikami tego samego systemu operacyjnego:
