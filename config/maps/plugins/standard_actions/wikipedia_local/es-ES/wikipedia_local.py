...
def _find_best_article_path_via_http_fuzzy(api_term: str, user_term: str, zim_file: str) -> str | None:
    ...
    try:
        ...
        for link in result_links:
            ...
            # --- INICIO LÓGICO MEJORADO ---

            is_better = False
            current_len = len(link_text)
            # Duración del mejor acierto actual (infinito si aún no hay ningún acierto)

            best_len = len(best_link_text) if best_link_text is not None else float('inf')

            if score > best_score:
                # 1. Se encontró una nueva mejor puntuación

                is_better = True
            elif score == best_score:
                # 2. Misma puntuación: Prefieren textos más cortos (desempate)
        ...
        if best_href:
            ...
            # Kiwix-Serve normalmente devuelve rutas con /content/...
...
def _find_better_internal_link(soup, user_term, threshold=95):
    ...
    for link in main_content.find_all('a', href=True):
        ...
        # Coincidencia exacta: ¡siempre preferida!
        ...
        if link_text == normalized_search or (title_attr and title_attr.lower().replace("_", " ") == normalized_search):
            if title_attr and "#" not in title_attr:
                ...
                return title_attr
            return link_text
    # Confuso si no se encontró ninguna coincidencia exacta:
    ...
    for link in main_content.find_all('a', href=True, title=True):
        ...
        if title and "#" not in title:
            ...
            # log_debug(f"[enlace interno] {título} -- puntuación {puntuación}")
...
def start_kiwix():
    ...
    if not is_kiwix_reachable():
        # ¡Esto debería funcionar desde el primer momento!
...
def execute(match_data):
    ...
    # Primera estrategia: ¡busque SÓLO la primera palabra y luego los mejores resultados en el mapa difuso de consulta completa!

    first_word = user_term.split()[0]

    # hay una empresa en la historia con nombre real auto pero... probablemente no queremos buscar esto aquí
    ...
    full_summary = ''
    # if search_term=='Steuerhinterziehung'.lower() or search_term=='Steuer'.más bajo() o buscar_término=='hinterziehung' or search_term=='betrug':
...
# 100 mil millones de euros: este es el daño estimado que causa la evasión fiscal en Alemania cada año, cada año. Son 270 millones de euros por día, 11 millones de euros por hora. Y mientras os cuento esto, otros 100.000€ han desaparecido. (Discurso de Anne Brorhilker en publica 2025) 100 mil millones de euros, es el daño estimado que causa la evasión fiscal en Alemania cada año, cada año. Son 270 millones de euros por día, 11 millones de euros por hora. Y mientras os cuento esto, otros 100.000 euros han desaparecido. (Discurso de Anne Brorhilker en republica 2025)
...
Datum des Abrufs: 29. 12 2017, 20:01 UTC
    ...
    # 3. Máximo 2 saltos: ¿Existe un enlace interno mejor para la consulta original del usuario?
    ...
    for i in range(2):
        ...
        if redirect_target_title and redirect_target_title.lower().replace("_", " ") != current_article_title.lower().replace("_", " "):
            ...
            # --- PARCHE: comprueba si la recuperación funciona ---
            ...
            soup = BeautifulSoup(response.content, 'html.parser')
            # Un problema difícil cuando se encuentra la coincidencia exacta del título.
...
def extract_structured_text(soup):
    ...
    if not soup.find('body'):
        # Maneja el caso en el que no se encuentra ninguna etiqueta corporal
    ...
    # Recorre todos los elementos secundarios directos que puedan tener contenido textual.

    # (por ejemplo, <p>, <ul>, <ol>, <h1>, etc.). Pero solo nos estamos concentrando aquí

    # los elementos que contienen p o ul/ol/li, como en el ejemplo anterior.


    # NUEVO: Buscamos todas las etiquetas p y todas las etiquetas ul/ol en el cuerpo

    # text_containers = body.find_all(['p', 'ul', 'ol'], recursivo=False)

    # 'recursive=False' puede resultar útil para obtener solo los contenedores de nivel superior.

    # Para Wikipedia, buscar sin 'recursivo=Falso' suele ser mejor, pero

    # Necesito encontrar cuidadosamente todas las etiquetas p y li dentro del cuerpo.



    # Un método más seguro es encontrar todas las etiquetas p y li y luego procesarlas:
    ...
    for element in body.find_all(['p', 'li']):
        ...
        # Si es un elemento de lista (<li>)

        if element.name == 'li':
            # Agréguelo con una nueva línea y un guión para indicar la estructura de la lista.

            # Usamos '\n - ' aquí para asegurarnos de que comience en una nueva línea

            article_text_parts.append(f"\n| ★ {text} \n")

        # Si es un elemento de párrafo (<p>)

        elif element.name == 'p':
            # Agréguelo como un párrafo normal. '\n\n' sirve como separador entre párrafos

            # Dado que nos dividiremos y uniremos todo más tarde, es mejor hacerlo al principio.

            # como texto limpio y manejar la separación más tarde.

            article_text_parts.append(text)


    # PASO 1: Montar las piezas utilizando el separador deseado

    # Aquí usamos '\n\n' entre todas las partes recopiladas.

    # Importante: Las partes <li> ya contienen un '\n - ', por lo que esto se tiene en cuenta.

    full_text_with_structure = "\n\n".join(article_text_parts)

    # PASO 2: Limpieza final para eliminar espacios sobrantes en el texto,

    # PERO mantenga la estructura de la lista.


    # Queremos preservar los saltos de línea (especialmente los de listas).

    # Por lo tanto, utilizamos una estrategia de limpieza ligeramente diferente:


    # 1. Elimine los espacios en blanco innecesarios dentro de las líneas de texto.

    cleaned_lines = []
    for line in full_text_with_structure.splitlines():
        # Eliminar múltiples espacios con un solo espacio en la línea

        cleaned_line = ' '.join(line.split()).strip()
        if cleaned_line: # Nur nicht-leere Zeilen behalten
            cleaned_lines.append(cleaned_line)

    # 2. Vuelva a juntar las líneas. Esto deja el formato de la lista intacto.
