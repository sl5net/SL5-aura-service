...
def _find_best_article_path_via_http_fuzzy(api_term: str, user_term: str, zim_file: str) -> str | None:
    ...
    try:
        ...
        for link in result_links:
            ...
            # --- DÉMARRAGE LOGIQUE AMÉLIORÉ ---

            is_better = False
            current_len = len(link_text)
            # Durée du meilleur hit actuel (infini s'il n'y a pas encore de hit)

            best_len = len(best_link_text) if best_link_text is not None else float('inf')

            if score > best_score:
                # 1. Nouveau meilleur score trouvé

                is_better = True
            elif score == best_score:
                # 2. Même score : Préférer les textes plus courts (tie-break)
        ...
        if best_href:
            ...
            # Kiwix-Serve renvoie généralement les chemins avec /content/...
...
def _find_better_internal_link(soup, user_term, threshold=95):
    ...
    for link in main_content.find_all('a', href=True):
        ...
        # Correspondance exacte – toujours préférée !
        ...
        if link_text == normalized_search or (title_attr and title_attr.lower().replace("_", " ") == normalized_search):
            if title_attr and "#" not in title_attr:
                ...
                return title_attr
            return link_text
    # Flou si aucune correspondance exacte n'a été trouvée :
    ...
    for link in main_content.find_all('a', href=True, title=True):
        ...
        if title and "#" not in title:
            ...
            # log_debug(f"[lien interne] {titre} -- score {score}")
...
def execute(match_data):
    ...
    # 1ère stratégie : recherchez UNIQUEMENT le premier mot, puis les meilleurs résultats sur la carte floue de requête complète !

    first_word = user_term.split()[0]

    # il y a une entreprise dans l'histoire avec son vrai nom auto mais... nous ne voulons probablement pas chercher cela ici
...
# 100 milliards d’euros, c’est le montant estimé des dégâts causés chaque année par l’évasion fiscale en Allemagne. Cela représente 270 millions d'euros par jour, 11 millions d'euros par heure. Et pendant que je vous dis cela, 100 000 € supplémentaires ont disparu. (Discours d'Anne Brorhilker in publica 2025) 100 milliards d'euros, c'est le préjudice estimé chaque année, chaque année, par l'évasion fiscale en Allemagne. Cela représente 270 millions d'euros par jour, 11 millions d'euros par heure. Et pendant que je vous dis cela, 100 000 euros supplémentaires ont disparu. (Discours d'Anne Brorhilker dans re-publica 2025)
...
Datum des Abrufs: 29. 12 2017, 20:01 UTC
    ...
    # 3. Max 2 sauts : existe-t-il un meilleur lien interne pour la requête utilisateur d'origine ?
    ...
    for i in range(2):
        ...
        if redirect_target_title and redirect_target_title.lower().replace("_", " ") != current_article_title.lower().replace("_", " "):
            ...
            # --- PATCH : vérifiez si la récupération fonctionne ---
            ...
            soup = BeautifulSoup(response.content, 'html.parser')
            # pause difficile lorsque la correspondance exacte du titre est trouvée
...
def extract_structured_text(soup):
    ...
    if not soup.find('body'):
        # Gère le cas où aucune balise body n'est trouvée
    ...
    # Parcourez tous les éléments enfants directs susceptibles d'avoir du contenu textuel

    # (par exemple <p>, <ul>, <ol>, <h1>, etc.). Mais nous sommes juste concentrés ici

    # les éléments contenant p ou ul/ol/li, comme dans l'exemple précédent.


    # NOUVEAU : Nous recherchons toutes les balises p et toutes les balises ul/ol dans le corps

    # text_containers = body.find_all(['p', 'ul', 'ol'], récursif=False)

    # 'recursive=False' peut être utile pour obtenir uniquement les conteneurs de niveau supérieur.

    # Pour Wikipédia, une recherche sans « recursive=False » est souvent préférable, mais nous

    # devez trouver soigneusement toutes les balises p et li dans le corps.



    # Une méthode plus sûre consiste à rechercher toutes les balises p et li, puis à les traiter :
    ...
    for element in body.find_all(['p', 'li']):
        ...
        # S'il s'agit d'un élément de liste (<li>)

        if element.name == 'li':
            # Ajoutez-le avec une nouvelle ligne et un trait d'union pour indiquer la structure de la liste

            # Nous utilisons '\n -' ici pour nous assurer qu'il commence sur une nouvelle ligne

            article_text_parts.append(f"\n| ★ {text} \n")

        # S'il s'agit d'un élément de paragraphe (<p>)

        elif element.name == 'p':
            # Ajoutez-le comme un paragraphe normal. '\n\n' sert de séparateur entre les paragraphes

            # Puisque nous allons tout diviser et tout rejoindre plus tard, il est préférable de le faire au début

            # sous forme de texte propre et gérer la séparation plus tard.

            article_text_parts.append(text)


    # ÉTAPE 1 : Assemblez les pièces à l'aide du séparateur souhaité

    # Ici, nous utilisons '\n\n' entre toutes les pièces collectées.

    # Important : Les parties <li> contiennent déjà un '\n - ', cela est donc pris en compte.

    full_text_with_structure = "\n\n".join(article_text_parts)

    # ÉTAPE 2 : Nettoyage final pour supprimer les espaces en excès dans le texte,

    # MAIS gardez la structure de la liste.


    # Nous souhaitons conserver les sauts de ligne (notamment ceux des listes).

    # Par conséquent, nous utilisons une stratégie de nettoyage légèrement différente :


    # 1. Supprimez les espaces inutiles dans les lignes de texte

    cleaned_lines = []
    for line in full_text_with_structure.splitlines():
        # Supprimer plusieurs espaces avec un seul espace sur la ligne

        cleaned_line = ' '.join(line.split()).strip()
        if cleaned_line: # Nur nicht-leere Zeilen behalten
            cleaned_lines.append(cleaned_line)

    # 2. Remettez les lignes ensemble. Cela laisse le formatage de la liste intact
