...
Debe tener el siguiente software instalado en su sistema Manjaro:
...
Asegúrese de que el servicio Docker esté instalado y ejecutándose.
...
Configure su entorno virtual Python e instale las bibliotecas necesarias.
...
### 3. Ejecutar el servidor Kiwix (la dependencia principal)
...
```
El servidor ahora se está ejecutando en `http://localhost:8080`.

### 4. El script de extracción de artículos

Cree un archivo llamado `article_extractor.py` y pegue en él el siguiente código de trabajo final.
...
2. **Detenga el servidor Docker (cuando haya terminado):**
Debe detener el contenedor Docker; de lo contrario, seguirá usando el puerto 8080.
    ```bash
