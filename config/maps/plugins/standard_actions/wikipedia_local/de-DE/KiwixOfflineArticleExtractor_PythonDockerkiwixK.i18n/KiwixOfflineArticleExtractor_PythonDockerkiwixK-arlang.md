...
يوفر هذا المستند دليلاً خطوة بخطوة لإعداد واستخدام برنامج Python النصي الذي يستخرج النص الكامل والنظيف لمقالة Wikipedia من ملف ZIM غير متصل بالإنترنت باستخدام خادم الويب `kiwix-serve` الذي يعمل داخل حاوية Docker.

                                      ### المتطلبات الأساسية

يجب أن يكون لديك البرنامج التالي مثبتًا على نظام Manjaro لديك:
...
3. **ملف ZIM:** قاعدة بيانات ويكيبيديا غير المتصلة بالإنترنت (على سبيل المثال، `wikipedia_de_all_mini_2025-09.zim`).
                   ...
                   تأكد من تثبيت خدمة Docker وتشغيلها.
...
قم بإعداد بيئة Python الافتراضية الخاصة بك وقم بتثبيت المكتبات اللازمة.
...
```
           ...
           ### 3. تشغيل خادم Kiwix (التبعية الأساسية)
...
```
           ...
           ### 4. البرنامج النصي لاستخراج المقالة

قم بإنشاء ملف باسم "article_extractor.py" والصق الكود العملي النهائي التالي فيه.
...
```
    ...
    ```
                   ...
                   2. **أوقف خادم Docker (عند الانتهاء):**
يجب عليك إيقاف حاوية Docker، وإلا فإنها ستستمر في استخدام المنفذ 8080.
    ```bash
