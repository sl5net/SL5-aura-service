## Extrator de artigos off-line Kiwix (Python/Docker/kiwix-serve)

Este documento fornece um guia passo a passo para configurar e usar um script Python que extrai o texto completo e limpo de um artigo da Wikipedia de um arquivo ZIM offline usando o servidor web `kiwix-serve` executado dentro de um contêiner Docker.

### Pré-requisitos

Você deve ter o seguinte software instalado em seu sistema Manjaro:

1. **Docker:** Para executar o servidor oficial `kiwix-tools` sem problemas de compilação.
2. **Python 3:** Com um ambiente virtual (`venv`).
3. **Um arquivo ZIM:** O banco de dados off-line da Wikipédia (por exemplo, `wikipedia_de_all_mini.zim`).

### 1. Configuração do sistema (Docker)

Certifique-se de que o serviço Docker esteja instalado e em execução.
...
Configure seu ambiente virtual Python e instale as bibliotecas necessárias.
...
### 3. Executando o servidor Kiwix (a dependência principal)

O script depende do `kiwix-serve` rodando na porta `8080`. Este comando usa a imagem oficial e estável do Docker e vincula seu diretório atual (contendo o arquivo ZIM) ao contêiner.

**IMPORTANTE:** Coloque seu arquivo ZIM (por exemplo, `wikipedia_de_all_mini.zim`) no diretório `kiwix_cli` antes de executar este comando.
...
```
O servidor agora está rodando em `http://localhost:8080`.

### 4. O script de extração de artigos

Crie um arquivo chamado `article_extractor.py` e cole o seguinte código funcional final nele.

__CODE_BLOCO_3__

### 5. Aplicação e limpeza
...
2. **Pare o servidor Docker (quando terminar):**
Você deve parar o contêiner do Docker, caso contrário ele continuará usando a porta 8080.
__CODE_BLOCO_5__
