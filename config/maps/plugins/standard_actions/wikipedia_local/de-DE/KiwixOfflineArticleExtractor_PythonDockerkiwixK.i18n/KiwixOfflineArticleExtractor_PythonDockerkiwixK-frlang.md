## Extracteur d'articles hors ligne Kiwix (Python/Docker/kiwix-serve)

Ce document fournit un guide étape par étape pour configurer et utiliser un script Python qui extrait le texte complet et propre d'un article Wikipédia à partir d'un fichier ZIM hors ligne à l'aide du serveur Web « kiwix-serve » exécuté dans un conteneur Docker.

### Prérequis

Vous devez avoir le logiciel suivant installé sur votre système Manjaro :
...
Assurez-vous que le service Docker est installé et exécuté.
...
Configurez votre environnement virtuel Python et installez les bibliothèques nécessaires.
...
```
Le serveur fonctionne désormais sur `http://localhost:8080`.

### 4. Le script d'extraction d'articles

Créez un fichier nommé « article_extractor.py » et collez-y le code final et fonctionnel suivant.
...
2. **Arrêtez le serveur Docker (une fois terminé) :**
Vous devez arrêter le conteneur Docker, sinon il continue d'utiliser le port 8080.
    ```bash
