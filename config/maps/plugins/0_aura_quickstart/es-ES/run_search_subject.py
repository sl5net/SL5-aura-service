...
def execute(match_data):
    ...
    # Colección: consagración

    docs_dir = None
    print('_________________________')
    print(f'dirpath_spoken: {dirpath_spoken}')
    assign_re = re.compile(r'^(?:configuración\w*|configuración\w*|configuración|consagración)$', re.IGNORECASE)
    m = assign_re.match(dirpath_spoken)
    if m:
        docs_dir = 'config'
        file_filter = "*.py"
    else:
        assign_re = re.compile(r'^(?:Documento\w*|documento\w*)$', re.IGNORECASE)
        if m:
            docs_dir = '~/Documents'
            file_filter = "*.md"
    ...
    if docs_dir == 'config':
        ...
        file_filter = "*.py"
# confederación rojaAura tímida ConfiguracionesConfiguración de búsqueda de DoraSobre tan buenas vibracionesO configuración de búsqueda
