...
'only_in_windows': ['Konsole', 'konsole', 'Console',
 ...
 {'command_flags': re.IGNORECASE}),
   ...
   {'command_flags': re.IGNORECASE, 'only_in_windows': ['Konsole', 'konsole', 'Terminal', 'Console']}),
    ...
    ('!git add . && git commit -m "WIP" && git push', r'^\s*(git|goes[^\s]*|go|grid|Gitta|Kate|kathe|kitten|fiat|with)\s+(add|at|did|dad|has|duet|slide|it|now|app)\s*(quick|fast|dirty|wip)?\s*$', 82,      {'command_flags': re.IGNORECASE, 'only_in_windows': ['Konsole', 'konsole', 'Terminal', 'Console']}),


    # EXAMPLE: git WIP push

    ('!git add . && git commit -m "WIP" && git push; && git ', r'^\s*(git|goes[^\s]*|go|grid|Gitta|Kate|kathe|kitten|fiat|with)\s*(quick|fast|dirty|wip)?\s*$', 82,      {'command_flags': re.IGNORECASE, 'only_in_windows': ['Konsole', 'konsole', 'Terminal', 'Console']}),
    ############################################
    ...
    ('git diff ', r'^\s*(kit|git|goes|peach)\s*(diff|deep|tiff|tuv|juice|tips|goes\'s|kittys|dies|die)\s*$', 75,      {'command_flags': re.IGNORECASE, 'only_in_windows': ['Konsole', 'konsole', 'Terminal', 'Console']}),
...
# please write to me because it will come with text'
    ...
    ("alias release_protokoll='gh release list --limit 100 | awk \"{print $1}\" | while read tag; do if [ -n \"$tag\" ]; then echo -e \"\n\n--- RELEASE: $tag ---\n\"; gh release view \"$tag\"; fi; done > all_releases.txt && kate all_releases.txt'", r'\b(releases\w* protocol\w*|relay\w* Protocols|all releases|releases\w* export\w*|fries Protocols)\b$', 75,      {'command_flags': re.IGNORECASE, 'only_in_windows': ['Konsole', 'konsole', 'Terminal', 'Console']}),
