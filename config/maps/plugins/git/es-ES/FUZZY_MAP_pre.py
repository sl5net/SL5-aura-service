...
'only_in_windows': ['Konsole', 'konsole', 'Console',
 ...
 {'command_flags': re.IGNORECASE}),
   ...
   {'command_flags': re.IGNORECASE, 'only_in_windows': ['Konsole', 'konsole', 'Terminal', 'Console', 'gnome-terminal', 'xterm', 'tilix', 'terminator', 'iTerm2', 'iTerm', 'Alacritty', 'kitty', 'Warp', 'Ghostty']}),
    ...
    ('!git add . && git commit -m "WIP" && git push', r'^\s*(git|va[^\s]*|ir|red|gita|kate|kathe|gatito|fíat|con)\s+(agregar|en|hizo|papá|tiene|dueto|deslizar|él|ahora|aplicación)\s*(rápido|rápido|sucio|limpiar)?\s*$', 82,      {'command_flags': re.IGNORECASE, 'only_in_windows': ['Konsole', 'konsole', 'Terminal', 'Console', 'gnome-terminal', 'xterm', 'tilix', 'terminator', 'iTerm2', 'iTerm', 'Alacritty', 'kitty', 'Warp', 'Ghostty']}),


    # EXAMPLE: git WIP empujar

    ('!git add . && git commit -m "WIP" && git push; && git ', r'^\s*(git|va[^\s]*|ir|red|gita|kate|kathe|gatito|fíat|con)\s*(rápido|rápido|sucio|limpiar)?\s*$', 82,      {'command_flags': re.IGNORECASE, 'only_in_windows': ['Konsole', 'konsole', 'Terminal', 'Console', 'gnome-terminal', 'xterm', 'tilix', 'terminator', 'iTerm2', 'iTerm', 'Alacritty', 'kitty', 'Warp', 'Ghostty']}),
    ############################################

    # (f'cd {CONFIG_DIR}/../../../../../; !git add . && git commit -m "WIP" && git push', r'^\s*(git|go[^\s]*|go|gitter|Gitta|kate|käthe|kitte|fiat|mit)\s+(add|at|tat|dad|hat|duett|rutsch|es|now|app)\s*(quick|fast|dirty|wip)?\s*$', 82, {'command_flags': re.IGNORECASE, 'only_in_windows': ['consola', 'consola', 'Terminal', 'Consola', 'iTerm2', 'iTerm', 'Alacritty', 'kitty', 'Warp', 'Ghostty']}),


    # (f'cd {CONFIG_DIR}/../../../../../; !git add . && git commit -m "WIP" && git push', r'^\s*(git|go[^\s]*|go|gitter|Gitta|kate|käthe|kitte|fiat|with)\s*(quick|fast|dirty|wip)?\s*$', 82, {'command_flags': re.IGNORECASE, 'only_in_windows': ['Konsole', 'Konsole', 'Terminal', 'Console', 'iTerm2', 'iTerm', 'Alacritty', 'kitty', 'Warp', 'Ghostty']}),
    ...
    ('git diff ', r'^\s*(equipo|git|va|durazno)\s*(diff|profundo|pelea|tuv|jugo|consejos|va\'s|kittys|dies|die)\s*$', 75,      {'command_flags': re.IGNORECASE, 'only_in_windows': ['Konsole', 'konsole', 'Terminal', 'Console', 'gnome-terminal', 'xterm', 'tilix', 'terminator', 'iTerm2', 'iTerm', 'Alacritty', 'kitty', 'Warp', 'Ghostty']}),
...
# por favor escríbeme porque vendrá con texto'
    ...
    ("alias release_protokoll='gh release list --limit 100 | awk \"{print $1}\" | while read tag; do if [ -n \"$tag\" ]; then echo -e \"\n\n--- RELEASE: $tag ---\n\"; gh release view \"$tag\"; fi; done > all_releases.txt && kate all_releases.txt'", r'\b(lanzamientos\w* protocolo\w*|relé\w* Protocolos|todo lanzamientos|lanzamientos\w* exportar\w*|papas fritas Protocolos)\b$', 75,      {'command_flags': re.IGNORECASE, 'only_in_windows': ['Konsole', 'konsole', 'Terminal', 'Console', 'gnome-terminal', 'xterm', 'tilix', 'terminator', 'iTerm2', 'iTerm', 'Alacritty', 'kitty', 'Warp', 'Ghostty']}),
