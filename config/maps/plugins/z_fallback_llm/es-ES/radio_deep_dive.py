...
VERSION = "1.1.0"
# AUTOR: Asistente de IA para sl5net
...
us this somwhre:
        ...
        # mod_thread = hablar (speech_text202603111649, bloqueo = Falso, use_espeak = Verdadero)
...
def get_markdown_context(f_path, max_headers=2):
    ...
    headers = []
    try:
        with open(f_path, 'r', codificación='utf-8') as f:
            for line in f:
                # Busca títulos de Markdown (por ejemplo, #Título o ##Subtítulo)

                match = re.match(r'^#+\s+(.*)', line)
                if match:
                    # Limpiamos los encabezados de cualquier enlace o formato de Markdown.
...
def generate_announcement_text(f_path): # f_path
    ...
    # Diferentes variantes para empezar
    ...
    # Lista de "dichos de los desarrolladores" para variar
    ...
    # Inserte el nombre del archivo (pronunciado de forma un poco más natural)

    file_intro = f"Es geht um die Datei {context['filename']}."

    # Incluya los títulos si están disponibles.
...
def _load_model_from_config():
    ...
    # la radio se encuentra en config/maps/plugins/z_fallback_llm/de-DE/

    # 5 niveles arriba = raíz del repositorio
...
def is_allowed_language(file_path):
    ...
    pattern = r'[-_]([a-z]{2})(?:[-_]|largo|\.Maryland$)'
    matches = re.findall(pattern, filename)

    if not matches:
        return True  # Kein Kürzel → verwenden

    # Sólo excluir si claramente NO de/en
...
def get_files_needing_update(root_dir):
    ...
    for root, dirs, files in os.walk(root_dir):
        # Filtro: excluir carpetas ocultas, de subrayado y de ruido
    ...
    for f_path in all_md_files:
        try:
            ...
            # Lógica: si el archivo es desconocido o disk-mtime es más nuevo que DB-mtime
...
def save_to_aura_db(question, answer, file_path):
    ...
    # elif "SL5-aura-service/" en file_path:
    ...
    try:
        ...
        # 1. Guarde el contenido en el caché de aura
        ...
        # 2. Actualizar la tabla de seguimiento
...
# env.setdefault("PANTALLA", ":0")

# env.setdefault("DBUS_SESSION_BUS_ADDRESS", "unix:path=/run/user/1000/bus")

# subproceso.Popen(["xdg-open", url], start_new_session=True, env=env)
...
def get_validated_github_url(stored_path):
    ...
    """
    # 1. Encuentre la raíz del proyecto de forma segura (su método /tmp/)
    ...
    try:
        ...
        # Alternativa si falta el archivo

        return None

    # 2. Normalizar ruta (~ expandir)

    full_path = Path(os.path.expanduser(stored_path))

    # 3. BÚSQUEDA GENERATIVA: Si el archivo no existe en la ubicación anterior

    if not full_path.exists():
        filename = full_path.name
        # Buscamos recursivamente en el proyecto el nombre del archivo

        # Ignoramos las carpetas .git y .venv por motivos de rendimiento.
        ...
        if search_results:
            # Damos el primer golpe que existe.

            full_path = search_results[0]
        else:
            # Realmente no se puede encontrar el archivo

            return None

    # 4. Cree la URL de GitHub a partir de la ruta (posiblemente nueva)

    try:
        # Calcular la ruta relativa a la raíz del proyecto

        rel_path = full_path.relative_to(project_root)
        # Construir enlace de GitHub
...
def get_github_url(stored_path):
    ...
    # 1. Busque el archivo localmente (autocurativo como de costumbre)
    ...
    # 2. COMPROBAR: ¿Git conoce siquiera este archivo?

    # Le preguntamos a Git si el archivo está en el índice (es decir, podría estar en línea)
    ...
    try:
        ...
        if check.returncode != 0:
            # El archivo no está en Git (ignorado o aún no agregado)
...
def main():
    ...
    with open(target, 'r', codificación='utf-8') as f:
        ...
        # ✅ Gasta PRIMERO, LUEGO lee en voz alta

        print(f"\n🤖 MODERATOR: {question}")
        sys.stdout.flush()
        # hablar(texto, voz="de-de", tono=50, bloqueo=False, use_espeak=False):

        mod_thread = speak(question, blocking=False,use_espeak=True)

        # 3. Ronda de expertos en IA
...
def DEMO_MODE():
    ...
    # 1. Abra el documento en el navegador (si existe el enlace y el modo navegador está activado)
...
if __name__ == "__main__":
    ...
    if settings.radio_deep_dive_ENABLED:
        ...
        parser.add_argument('--no-browser', acción='store_true')
