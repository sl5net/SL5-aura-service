...
# Su lógica SL5NET_AURA_PROJECT_ROOT

tmp_dir = Path("C:/tmp") if os.name == "nt" else Path("/tmp")
SL5NET_AURA_PROJECT_ROOT = get_aura_project_root()

# Agregue la ruta al directorio de complementos actual para importaciones absolutas

PLUGIN_DIR = str(SL5NET_AURA_PROJECT_ROOT / "config" / "maps" / "plugins" / "z_fallback_llm" / "de-DE")
if PLUGIN_DIR not in sys.path:
    sys.path.insert(0, PLUGIN_DIR)

# Ahora las importaciones absolutas funcionan sin el punto (.)
...
# utils.log_debug(f"🚀 inicio 4/12/25 4:45 p.m. jueves")

# utils.log_debug(f"🚀 inicio 4/12/25 5:12 p.m. jueves")
...
# Asegurándonos de encontrar Ask_ollama
...
# print("❌ ERROR: No se pudo importar 'ask_ollama.py'.")
...
# --- OBJETO SIMULADO (Para que Aura piense que viene del micrófono) ---

class MockMatchObj:
    def __init__(self, text):
        self.text = text
    def groups(self):
        # return("Computadora", self.text)
...
def generate_user_question(last_aura_response, round_num):
    ...
    system_prompt_LinuxAdministrator = ( # noqa: F841
        ...
        "Kontext: Aura ist headless, nutzt '/tmp/sl5_record.trigger' y Configuraciones en 'config/maps/'.\n\n"
    ...
    # Dando contexto: ¿Qué acaba de decir Aura?

    context_prompt = f"{system_prompt}\n\nLETZTE ANTWORT DES ASSISTENTEN:\n\"{last_aura_response}\"\n\nDEINE NÄCHSTE FRAGE:"

    # cmd = ["ollama", "ejecutar", "llama3.2"]
    ...
    # Limpiar (A veces Llama habla mucho)
    ...
    # Asegúrate de que "Computadora" esté al principio (en caso de que Llama lo olvide)
...
def speak_espeak(text):
    ...
    # h = os.environ.get("INICIO", "/tmp") # variable $INICIO

    # f = "/tmp/sl5_aura/simulate_conversation.txt"

    # proceso = subproceso.run(['python', Speak_file_path, f], stdout=subproceso.DEVNULL, stderr=subproceso.DEVNULL)
...
def speak(text):
    """Gibt Text über ein TTS-System aus. Passen Sie den Befehl ggf. an."""
    try:
        # subproceso.run(['espeak', '-v', 'de', texto], check=True)
        ...
        # --- REVISIÓN: silenciar explícitamente la salida estándar ---
...
# --- BOT B: AURA (Ollama 2 + Lógica Python) ---

def ask_aura(question):
    """
    Ruft das echte Aura-Plugin auf.
    """
    # imprimir(f"🎤 ENTRADA: '{pregunta}'")


    match_data = {'regex_match_obj': MockMatchObj(question)}

    start = time.time()
    # Aquí es donde ocurre la magia (caché, búsqueda Léame, etc.)
...
def main():
    ...
    for i in range(1, ROUNDS + 1):
        ...
        if psutil.virtual_memory().percent >90:
            # reiniciar su script es una solución alternativa muy común y efectiva para administrar el uso excesivo de memoria
        ...
        # 1. El usuario genera una pregunta.
        ...
        # CÁLCULOS DE PRODUCCIÓN (al final de cada ronda)

        # -------------------------------------------------------


        # 1. Plazo anterior (transcurrido)

        elapsed_time_secs = time.time() - script_start_time

        # 2. Tiempo promedio por ronda (evitando dividir por cero cuando i=1)

        # Si i > 0, calcular el promedio, de lo contrario 0

        avg_time_per_round = elapsed_time_secs / i if i > 0 else 0

        # 3. Tiempo total estimado (para todas las RONDAS)

        estimated_total_time_secs = avg_time_per_round * ROUNDS

        # 4. Tiempo restante estimado

        remaining_time_secs = estimated_total_time_secs - elapsed_time_secs

        # 5. Fin estimado (marca de tiempo)

        estimated_end_timestamp = script_start_time + estimated_total_time_secs

        # 6. Formatear la hora de finalización en una cadena legible

        estimated_end_str = datetime.datetime.fromtimestamp(estimated_end_timestamp).strftime('%H:%M:%S')

        # 7. Formatear las duraciones de tiempo en una cadena legible (minutos, segundos)

        # Aquí use la función format_duration que discutimos anteriormente.

        # (Supongo que esto está disponible; de lo contrario, utilice el redondeo simple)
...
if __name__ == "__main__":
    ...
    # psutil.virtual_memory().porcentaje
