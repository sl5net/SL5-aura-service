...
def get_markdown_context(f_path, max_headers=2):
    ...
    try:
        with open(f_path, 'r', encoding='utf-8') as f:
            for line in f:
                # Searches for Markdown headings (e.g. #Title or ##Subtitle)

                match = re.match(r'^#+\s+(.*)', line)
                if match:
                    # We clean the headers of any Markdown links or formatting
...
def generate_announcement_text(f_path): # f_path
    ...
    # Different variants to get you started
    ...
    # List of “developer sayings” for variety
    ...
    # Insert the file name (pronounced a little more naturally)

    file_intro = f"Es geht um die Datei {context['filename']}."

    # Include the headings if available
...
def _load_model_from_config():
    ...
    # radio is located in config/maps/plugins/z_fallback_llm/de-DE/
...
def is_allowed_language(file_path):
    ...
    pattern = r'[-_]([a-z]{2})(?:[-_]|long|\.md$)'
    matches = re.findall(pattern, filename)

    if not matches:
        return True  # Kein Kürzel → verwenden

    # Only exclude if clearly NOT de/en
...
def get_validated_github_url(stored_path):
    ...
    """
    # 1. Find the project root safely (your /tmp/ method)
    ...
    # 2. Normalize path (~expand)

    full_path = Path(os.path.expanduser(stored_path))

    # 3. GENERATIVE SEARCH: If file does not exist in the old location

    if not full_path.exists():
        filename = full_path.name
        # We search recursively in the project for the file name

        # We ignore .git and .venv folders for performance
        ...
        if search_results:
            # We take the first hit that exists

            full_path = search_results[0]
        else:
            # File really cannot be found

            return None

    # 4. Build GitHub URL from the (possibly new) path
...
def get_github_url(stored_path):
    ...
    # 1. Find the file locally (self-healing as usual)
    ...
    # 2. CHECK: Does Git even know this file?

    # We ask Git if the file is in the index (i.e. could be online)
    ...
    try:
        ...
        if check.returncode != 0:
            # File is not in Git (ignored or not added yet)
...
def main():
    ...
    with open(target, 'r', encoding='utf-8') as f:
        ...
        # ONLY open when the switch is active
...
def DEMO_MODE():
    ...
    # 1. Open the document in the browser (if link exists and browser mode on)
