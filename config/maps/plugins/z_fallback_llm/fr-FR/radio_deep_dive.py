...
VERSION = "1.1.0"
# AUTEUR : AI Assistant pour sl5net
...
us this somwhre:
        ...
        # mod_thread = parler(speech_text202603111649, blocking=False,use_espeak=True)
...
def get_markdown_context(f_path, max_headers=2):
    ...
    headers = []
    try:
        with open(f_path, 'r', codage='utf-8') as f:
            for line in f:
                # Recherche des titres Markdown (par exemple #Title ou ##Subtitle)

                match = re.match(r'^#+\s+(.*)', line)
                if match:
                    # Nous nettoyons les en-têtes de tout lien ou formatage Markdown
...
def generate_announcement_text(f_path): # f_path
    ...
    # Différentes variantes pour commencer
    ...
    # Liste des « dictons des développeurs » pour la variété
    ...
    # Insérez le nom du fichier (prononcé un peu plus naturellement)

    file_intro = f"Es geht um die Datei {context['filename']}."

    # Inclure les titres si disponibles
...
def _load_model_from_config():
    ...
    # la radio se trouve dans config/maps/plugins/z_fallback_llm/de-DE/

    # 5 niveaux plus haut = racine du dépôt
...
def is_allowed_language(file_path):
    ...
    pattern = r'[-_]([a-z]{2})(?:[-_]|long|\.Maryland$)'
    matches = re.findall(pattern, filename)

    if not matches:
        return True  # Kein Kürzel → verwenden

    # Exclure seulement si clairement PAS de/en
...
def get_files_needing_update(root_dir):
    ...
    for root, dirs, files in os.walk(root_dir):
        # Filtrer : exclure les dossiers masqués, soulignés et bruyants
    ...
    for f_path in all_md_files:
        try:
            ...
            # Logique : si le fichier est inconnu ou si disk-mtime est plus récent que DB-mtime
...
def save_to_aura_db(question, answer, file_path):
    ...
    # Logique du lien GitHub

    # chemin_rel = ""

    # si "STT/" dans file_path :

    # chemin_rel = chemin_fichier.split("STT/")[1]

    # elif "SL5-aura-service/" dans chemin_fichier :
    ...
    try:
        ...
        # 1. Enregistrez le contenu dans le cache aura
        ...
        # 2. Mettre à jour le tableau de suivi
...
def get_validated_github_url(stored_path):
    ...
    """
    # 1. Trouvez la racine du projet en toute sécurité (votre méthode /tmp/)
    ...
    try:
        ...
        # Repli si le fichier est manquant

        return None

    # 2. Normaliser le chemin (~ développer)

    full_path = Path(os.path.expanduser(stored_path))

    # 3. RECHERCHE GÉNÉRATIVE : Si le fichier n'existe pas à l'ancien emplacement

    if not full_path.exists():
        filename = full_path.name
        # On recherche récursivement dans le projet le nom du fichier

        # Nous ignorons les dossiers .git et .venv pour des raisons de performances
        ...
        if search_results:
            # On prend le premier coup qui existe

            full_path = search_results[0]
        else:
            # Le fichier est vraiment introuvable

            return None

    # 4. Créez l'URL GitHub à partir du chemin (éventuellement nouveau)

    try:
        # Calculer le chemin par rapport à la racine du projet

        rel_path = full_path.relative_to(project_root)
        # Créer un lien GitHub
...
def get_github_url(stored_path):
    ...
    # 1. Recherchez le fichier localement (auto-réparation comme d'habitude)
    ...
    # 2. VÉRIFIER : Git connaît-il ce fichier ?

    # Nous demandons à Git si le fichier est dans l'index (c'est-à-dire s'il pourrait être en ligne)
    ...
    try:
        ...
        if check.returncode != 0:
            # Le fichier n'est pas dans Git (ignoré ou pas encore ajouté)
...
def main():
    ...
    with open(target, 'r', codage='utf-8') as f:
        ...
        # ✅ Dépensez D'ABORD, PUIS lisez à haute voix
...
def DEMO_MODE():
    ...
    # question, réponse = ligne



    question, answer, doc_url = row

    # 1. Ouvrez le document dans le navigateur (si le lien existe et le mode navigateur activé)
