...
def check_db_statistics_and_exit_if_invalid():
    ...
    try:
        ...
        if unique_prompts == 0:
            diagnosis = f"{DB_FILE} ist LEER. Es sind 0 eindeutige Fragen vorhanden. sqlitebrowser {DB_FILE} & "
        ...
        print(f"PRÜFUNG: Ist die Datenbank '{DB_FILE}' die richtige Datei?")
        if conn:
            conn.close()
