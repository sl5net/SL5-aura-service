...
# configmaps/koans german/05_koan_such example/de-DE/FUZZY_MAP_pre.py
...
# Rules can execute external Python scripts.

# Here: database search via voice command.

#
# TASK:

# Say: "Look in Ruth chapter 1 verse 1"

#
# EXPECTED RESULT:

# Aura runs bible_search.py and prints the verse.
...
FUZZY_MAP_pre = [
    ...
    # EXAMPLE: search in x text chapter 123 vfdph text 123

    ('bible suche', r'^search in (?P<book>\w*[ ]?\w+) chapter (?P<chapter>\d+) [vfdph]\w+ (?P<verses>\d+)$', 90, {
        'command_flags': re.IGNORECASE,
        'on_match_exec': [CONFIG_DIR / 'bible_search.py']
    }),

    # TODO: Could you invent some other search options?
