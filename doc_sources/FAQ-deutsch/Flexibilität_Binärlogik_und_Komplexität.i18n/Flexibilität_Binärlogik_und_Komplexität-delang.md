...
* **Kumulation:** Durch die Kumulation können mehrere Regeln sequenziell auf den Text angewendet werden, um beispielsweise die Benennungskonventionen oder Platzhalter eines Frameworks schrittweise in eine gewünschte Ausgabe zu führen.
* **Plugins:** Die hierarchische Ladelogik über `plugins/` stellt sicher, dass projektspezifische oder Frameworkspezifische Regeln (z. B. für CodeIgniter) als separates Modul hinzugefügt werden können. Diese Plug-in-Regeln haben eine definierte, aber niedrigere Priorität als die Kern-Sprachregeln, wodurch die Kernlogik unverändert bleibt, aber das Verhalten gezielt erweitert werden kann.
