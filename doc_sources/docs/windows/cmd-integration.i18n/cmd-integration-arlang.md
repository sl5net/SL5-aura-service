...
- **`bash` للبرنامج النصي Kiwix**: إذا تم تثبيت WSL، فسيكون `bash` متاحًا في CMD وسيتم تشغيل البرنامج النصي `.sh` مباشرة. بخلاف ذلك، قم بتعديل `kiwix-docker-start-if-not-running.sh` إلى ما يعادل `.bat`.
