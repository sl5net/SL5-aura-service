...
> ⚠️ 이 모듈은 기능적이지만 철저하게 테스트되지는 않았습니다. 다음과 같은 경우에는 제대로 작동하지 않을 수 있습니다.

- **숫자** – '5' 또는 '6'은 유효한 Python 식별자가 아니며 자동 수정 기능이 이를 처리하지 못할 수 있습니다.
- **특수 문자** – `-`, `.`, 움라우트가 있는 단어는 `NameError`를 트리거하지 않을 수 있습니다.
- **다중 단어 항목** – `thomas mueller`(공백 포함)로 인해 `NameError`가 아닌 `SyntaxError`가 발생하므로 자동 수정이 실행되지 않을 수 있습니다.
- **쉼표로 구분된 값** – `drei, vier`는 적절한 튜플이 되지 않고 그대로 삽입될 수 있습니다.

자동 수정이 실행되지 않으면 규칙을 수동으로 추가하세요.
```python
