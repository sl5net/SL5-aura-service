...
rm Z_LanguageTool-6.6.zip 2>/dev/null
python3 tools/download_release_hybrid.py --exclude all 2>&1
...
def apply_manifest_urls(assets: list, manifest: dict, port: str = "8829") -> list:
    ...
    for pkg in manifest.values():
        for fname, url in zip(pkg.get('files', []), pkg.get('urls', [])):
            if "aura.sl5.de" in url in url:
                # url = url.replace("aura.sl5.de", f"aura.sl5.de:{port}")
                url = re.sub(r"aura\.sl5\.de(:\d+)?", f"aura.sl5.de:{port}", url)
            url_map[fname] = url
    for asset in assets:
...
def download_via_torrent(torrent_url: str, save_path: str) -> bool:
    ...
    if r.status_code != 200:
        print(f"  Could not fetch .torrent file ({torrent_url}) (HTTP {r.status_code})")
        return False
    ...
    #     # Deaktiviere SSL-Check (falls verfügbar in deiner Version)
    #     if hasattr(lt.settings_pack, 'validate_https_certificates'):
    #         pack.set_bool(lt.settings_pack.validate_https_certificates, False)
    #
    #     ses = lt.session(pack)

    # bis 2026-0325-1842 hätte beihnahe funktioniert
    # try:
    #     decoded_dict = lt.bdecode(r.content)
    #     # Web-Seeds (url-list) auf Port 8829 umbiegen
    #     if b'url-list' in decoded_dict:
    #         urls = decoded_dict[b'url-list']
    #         if isinstance(urls, bytes): urls = [urls]
    #         new_urls = []
    #         for u in urls:
    #             u_str = u.decode('utf-8')
    #
    #             u_str = re.sub(r"aura\.sl5\.de(:\d+)?", f"aura.sl5.de:8829", u_str)
    #
    #             print(f"  DEBUG: Nutze Web-Seed: {u_str}")
    #
    #             new_urls.append(u_str.encode('utf-8'))
    #         decoded_dict[b'url-list'] = new_urls
    #
    #
    #     # ses  = lt.session({"listen_interfaces": "0.0.0.0:6881"})
    #     # Session-Settings erweitern:
    #     settings = {
    #         "listen_interfaces": "0.0.0.0:6881",
    #         "validate_https_certificates": False, # WICHTIG
    ...
    try:
        decoded_dict = lt.bdecode(r.content)
        # 1. URLs im Dictionary patchen (WICHTIG!)
        if b'url-list' in decoded_dict:
            new_urls = []
            for u in decoded_dict[b'url-list']:
                u_str = u.decode('utf-8')
                u_str = re.sub(r"aura\.sl5\.de(:\d+)?", "aura.sl5.de:8829", u_str)
                if not u_str.endswith("/"): u_str += "/"
                new_urls.append(u_str.encode('utf-8'))
            decoded_dict[b'url-list'] = new_urls

        # Session & Settings (Universal für v1.x und v2.x)
        if hasattr(lt, "settings_pack"):
            # Neues API (Manjaro / libtorrent 2.x)
            pack = lt.settings_pack()
            pack.set_str(lt.settings_pack.listen_interfaces, "0.0.0.0:6881")
            if hasattr(lt.settings_pack, "validate_https_certificates"):
                pack.set_bool(lt.settings_pack.validate_https_certificates, False)
            ...
            # Altes API (Ubuntu / libtorrent 1.x)
            ses = lt.session()
            settings = {
                "listen_interfaces": "0.0.0.0:6881",
                "validate_https_certificates": False,
            }
        ...
        ses  = lt.session(pack)
        info = lt.torrent_info(decoded_dict)
