...
SL5NET_AURA_PROJECT_ROOT = Path(project_root_file.read_text().strip())
print(f'SL5NET_AURA_PROJECT_ROOT: {SL5NET_AURA_PROJECT_ROOT}')
...
for (old_path,) in rows:
    try:
        new_path = str(Path(old_path).relative_to(SL5NET_AURA_PROJECT_ROOT))
