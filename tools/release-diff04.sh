...
# === CODE-DIFF ===============================================================
# Diff in temporäre Datei schreiben — vermeidet Subshell-Bug mit Variablen
echo "=== CODE ÄNDERUNGEN ==="

TMPFILE=$(mktemp)
trap "rm -f $TMPFILE" EXIT

git diff -U0 --no-color "${FROM_REF}..${TO_REF}" -- . "${EXCLUDE_ARGS[@]}" > "$TMPFILE"
...
while IFS= read -r line; do
  ...
  if [[ "$line" =~ ^\+[^+] ]]; then
    content="${line:1}"
    [ -z "${content// }" ] && continue
    # Kommentar/Import-Filter — läuft jetzt in der GLEICHEN Shell (kein Subshell-Bug)
...
done < "$TMPFILE"   # < statt | — keine Subshell, Variablen funktionieren korrekt
