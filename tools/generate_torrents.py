...
(url-list / BEP-19), so any standard client (qBittorrent, Deluge, ...) can
download directly from GitHub when no peers are available.

Dependencies: only the Python standard library + requests.
No libtorrent, no C extensions required.

Usage
-----
    python3 generate_torrents.py --tag v0.2.0

    # dry-run: list packages, write nothing
    python3 generate_torrents.py --tag v0.2.0 --list

    # with real local files for correct piece hashes (enables seeding)
    python3 generate_torrents.py --tag v0.2.0 --local-dir ~/downloads/v0.2.0

    # custom output directory
    python3 generate_torrents.py --tag v0.2.0 --out-dir ./torrents

    # upload result to the same release (requires gh CLI)
    gh release upload v0.2.0 torrents/*.torrent

Asset naming convention (sl5net/SL5-aura-service)
--------------------------------------------------
  Parts:      Z_<base>.part.<aa|ab|...>   -> one torrent per <base>
  Direct:     <base>  (no Z_, no .part.)  -> one torrent (single-file)
  Checksums:  <base>.sha256sums.txt       -> skipped
  Scripts:    *.py, *.sh                  -> skipped
  i18n:       anything with .i18n         -> skipped

Stub mode vs. real mode
-----------------------
Without --local-dir:  file sizes come from the GitHub API, piece hashes are
  computed over zero-byte data of the correct length.  The torrent is valid
  and clients can download via web-seed, but it cannot be used for seeding.
With --local-dir:  piece hashes are computed from the real files.  Both
  downloading and seeding work correctly.
"""
...
def compute_pieces(file_paths: list, file_sizes: list, piece_size: int) -> bytes:
    """
    Compute SHA1 piece hashes across all files in order.

    If a path exists on disk: read real bytes.
    Otherwise: simulate with zero bytes of the correct size (stub mode).
    Returns concatenated raw SHA1 digests (20 bytes each).
    """
    sha1s = b""
    buf   = b""
...
def build_torrent(
    ...
    """
    Build a torrent (BEP-3 + BEP-19 url-list).
    Single-file and multi-file are handled automatically.
    file_sizes: from local stat or from GitHub API asset.size
    """
...
def process_package(
    ...
    if parts:
        ...
        web_seeds   = [f"{base_url}/{direct['name']}"]
    ...
    if local_dir:
        ...
        if missing:
            print(f"    WARNING: {len(missing)} file(s) not found locally - stub hashes used for those.")
        else:
            print(f"    Hashing from local files...")
    else:
        file_paths = [""] * n
        file_sizes = api_sizes
        total_mb   = sum(file_sizes) / (1024 * 1024)
        print(f"    Stub mode (sizes from API: {total_mb:.0f} MB total) - web-seed download OK, seeding requires real files")
...
def main() -> None:
    ...
    if not args.list:
        ...
        print(f"  gh release upload {args.tag} {args.out_dir}/*.torrent")
