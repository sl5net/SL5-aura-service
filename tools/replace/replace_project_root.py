...
def main():
    target_str = sys.argv[1] if len(sys.argv) > 1 else "config/maps"
    target_dir = Path(target_str).resolve()
    print(f"Starting PROJECT_ROOT replacement in: {target_dir}")

    modified = process_directory(target_dir)
    print(f"\nReplacement completed. Total modified files: {len(modified)}")
    for p in modified:
        print(f"  - {p}")
