#!/usr/bin/env python3
"""Add macOS file managers to exclude_windows in z_fallback_llm map files."""
...
def transform_fallback_llm_file_managers(content: str) -> str:
    """Pure transformation function to insert Finder, ForkLift, Commander One into exclude_windows."""
    def replacer(match: re.Match) -> str:
        matched_str = match.group(0)
        keywords = ["double", "commander", "comandante", "cmd", "nemo"]
        if any(kw in matched_str.lower() for kw in keywords):
            if "finder" not in matched_str.lower():
                return re.sub(r"(\s*\])", r", r'finder', r'forklift', r'commander one'\1", matched_str)
        return matched_str
