#!/usr/bin/env python3
"""Add macOS compatible terminal emulators to git plugin map configurations."""
...
def transform_terminal_list(content: str) -> str:
    """Pure transformation function to insert macOS terminals into only_in_windows lists."""
    pattern = re.compile(
        r"('only_in_windows':\s*\[(?!.*'iTerm2')[^\]]*'Terminal'[^\]]*?)(\s*\])"
    )
    replacement = (
        r"\1, 'iTerm2', 'iTerm', 'Alacritty', 'kitty', 'Warp', 'Ghostty'\2"
    )
