...
PowerShell 또는 CMD를 **관리자**로 열고 다음을 실행합니다.
...
시스템 리소스를 절약하려면 백그라운드에서 지속적으로 실행되지 않고 필요할 때만(예: Aura가 Trino 데이터베이스를 요청할 때) Docker가 시작되도록 구성할 수 있습니다.

지속적인 백그라운드 서비스를 비활성화하고 대신 "소켓 활성화"를 활성화하려면 다음 명령을 실행하십시오.
...
**업데이트가 안전하도록 만드세요(Linux Arch/Manjaro 사용자):**
패키지 업데이트로 인해 소켓 구성이 재설정되는 경우가 있습니다. 이를 방지하려면 영구 로컬 재정의를 만드세요.
