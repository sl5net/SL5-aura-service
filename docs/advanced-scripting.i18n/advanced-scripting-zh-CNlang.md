...
* `'text_after_replacement'` (str)：应用规则的基本替换字符串*之后*但调用脚本*之前*的文本。 （如果替换为 ``，则与 `original_text` 相同）。
...
此示例展示了一个脚本如何通过检查用户的原始文本来处理多个命令（添加、显示），以及如何通过写入文件来保留数据。
