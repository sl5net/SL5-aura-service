...
لتسهيل التفاعل مع STT (تحويل الكلام إلى نص) CLI، يمكنك إضافة وظيفة اختصار إلى `~/.bashrc`. يتيح لك ذلك كتابة "سؤالك" في الوحدة الطرفية.
...
> **ملاحظة:** إذا كنت تستخدم Bash كواجهة تسجيل دخول (على سبيل المثال عبر SSH)، فأضف أيضًا نفس الكتلة إلى `~/.bash_profile`، أو المصدر `~/.bashrc` منها:
                                                                 > ``` باش
                                 > [ -f ~/.bashrc ] && المصدر ~/.bashrc
                                                                        > ```
