# Integração Bash Shell

Para facilitar a interação com a CLI STT (Speech-to-Text), você pode adicionar uma função de atalho ao seu `~/.bashrc`. Isso permite que você simplesmente digite `s "sua pergunta"` no terminal.
