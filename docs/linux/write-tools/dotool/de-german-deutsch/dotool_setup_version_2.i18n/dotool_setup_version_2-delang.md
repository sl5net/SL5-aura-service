...
## Was ist dotool?
„dotool“ ist ein Werkzeug zur Simulation von Tastatureingaben. Im Gegensatz zu „xdotool“ kommuniziert es direkt mit dem Kernel über „uinput“ und funktioniert daher universell unter **X11 und Wayland**.
...
Damit `dotool` ohne Root-Rechte tippen darf, muss dein Benutzer in der Gruppe `input` und eine udev-Regel aktiv sein:

1. **Benutzer zur Gruppe:** `sudo gpasswd -a $USER input`
2. **udev-Regel:**
   ```bash
