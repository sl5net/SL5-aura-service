...
### 2. Benutzer zur `input`-Gruppe hinzufügen
...
Einstellungen werden so ausgelesen, dass `print()`-Ausgaben in `settings.py` vorliegen
...
- **Umlaute und Sonderzeichen:** `type delay 2` (do tool-Default) wird empfohlen.
Bei „typedelay 0“ können Zeichen wie ä, ö, ü, ß verloren gehen.
- **Zu schnell für die Zielanwendung?** Manche Apps (z. B. Electron, Browser-Eingaben)
Zeichen verlieren bei niedrigem Delay. In diesem Fall `dotool_typedelay = 5` oder höher verwenden.
...
um den Overhead eines Prozesses bei jedem Tastendruck zu vermeiden.
...
- **Zu schnell für die Zielanwendung?** Manche Apps (z. B. Electron, Browser-Eingaben)
Zeichen verlieren bei `typedelay 0`. In diesem Fall `typedelay 5` oder `typedelay 10` verwenden.
- **Wayland:** dotool funktioniert auch unter Wayland, xdotool hingegen nicht.
- **Fallback:** Wenn dotool nicht installiert ist, fällt das Skript automatisch auf „xdotool“ zurück.