# Intégration Ksh (Korn Shell)

Pour faciliter l'interaction avec la CLI STT (Speech-to-Text), vous pouvez ajouter une fonction de raccourci à votre `~/.kshrc`. Cela vous permet de taper simplement « votre question » dans le terminal.

## Instructions de configuration

1. Ouvrez votre configuration Ksh avec un éditeur que vous aimez :
   ```bash
   nano ~/.kshrc
   kate ~/.kshrc
   ```
...
3. Assurez-vous que Ksh charge votre fichier de configuration. Ajoutez ou vérifiez ceci dans `~/.profile` :
   ```ksh
...
## Notes spécifiques à Ksh

- Ksh prend en charge à la fois la syntaxe `function name { }` et `name() { }` ; le mot-clé `function` est utilisé ici pour plus de clarté.
- `local` n'est **pas** pris en charge dans toutes les variantes de Ksh (par exemple `ksh88`). Les variables de la fonction ci-dessus sont donc déclarées sans « local ». Si vous utilisez `mksh` ou `ksh93`, `typeset` peut être utilisé à la place : `typeset TEMP_FILE=$(mktemp)`.
- La variable `ENV` contrôle quelles sources Ksh sont enregistrées pour les sessions interactives, similaire à `.bashrc`.
