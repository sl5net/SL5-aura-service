...
STT(Speech-to-Text) CLI와 더 쉽게 상호 작용하려면 셸 프로필에 바로가기 기능을 추가하면 됩니다. 이렇게 하면 터미널에 '질문'을 입력하기만 하면 됩니다.
...
- 이 구성 파일은 로그인 쉘에서 제공되는 `~/.profile`에 배치하는 것이 가장 좋습니다. 대화형 비로그인 셸의 경우 배포판에서 `~/.shrc`를 사용할 수 있습니다. 시스템 설명서를 확인하세요.
