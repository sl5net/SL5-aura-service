# Intégration du shell macOS Zsh

> **Shell par défaut depuis macOS Catalina (10.15).** Si vous utilisez macOS Mojave ou une version antérieure, consultez plutôt le guide [macOS Bash Integration](.././mac-bash-integration.i18n/mac-bash-integration-frlang.md).

Pour faciliter l'interaction avec la CLI STT (Speech-to-Text), vous pouvez ajouter une fonction de raccourci à votre `~/.zshrc`. Cela vous permet de taper simplement « votre question » dans le terminal.

## Instructions de configuration

1. Ouvrez votre configuration Zsh avec un éditeur que vous aimez :
   ```zsh
   nano ~/.zshrc
...
Après l'installation, `timeout` est disponible sous le nom `gtimeout`. Ajoutez un alias ou remplacez « timeout » par « gtimeout » dans la fonction ci-dessus :
  ...
  ```
Ajoutez l'alias au-dessus de la fonction `s()` dans votre `~/.zshrc`.

- **`pgrep`** est disponible sur macOS par défaut.
