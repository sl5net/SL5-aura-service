# تكامل MacOS Zsh Shell

> **الغلاف الافتراضي منذ macOS Catalina (10.15).** إذا كنت تستخدم macOS Mojave أو إصدارًا أقدم، فاطلع على دليل [macOS Bash Integration](.././mac-bash-integration.i18n/mac-bash-integration-arlang.md) بدلاً من ذلك.
...
بعد التثبيت، يتوفر `timeout` باسم `gtimeout`. قم بإضافة اسم مستعار أو استبدال `timeout` بـ`gtimeout` في الوظيفة أعلاه:
  ...
  ```
أضف الاسم المستعار أعلى الدالة `s()` في `~/.zshrc`.

- **`pgrep`** متاح على نظام التشغيل macOS بشكل افتراضي.

- **مسار بايثون**: تأكد من إعداد بيئتك الافتراضية على `$SL5NET_AURA_PROJECT_ROOT/.venv`. إذا كنت تدير لغة Python باستخدام `pyenv` أو `conda`، فاضبط `PY_EXEC` وفقًا لذلك.
