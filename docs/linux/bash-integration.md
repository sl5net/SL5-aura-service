...
To make interacting with the STT (Speech-to-Text) CLI easier, you can add a shortcut function to your `~/.bashrc`. This allows you to simply type `s "your question"` in the terminal.
...
> **Note:** If you use Bash as a login shell (e.g. via SSH), also add the same block to `~/.bash_profile`, or source `~/.bashrc` from it:
> ```bash
