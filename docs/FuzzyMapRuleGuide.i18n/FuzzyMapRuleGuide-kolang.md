...
### 파일 기반 교체
`replacement`가 `FILE4REPLACEMENT_ALLOWED_PREFIXES`에 구성된 접두사로 시작하는 경우
(기본값: `-` 또는 `.`), Aura는 이를 파일 이름으로 처리하고
리터럴 문자열을 사용하는 대신 플러그인의 디렉터리를 사용합니다. 이는 비밀(비밀번호,
API 키)를 소스 코드에서 가져옵니다.
자세한 내용은 [File_Based_Replacement.md](../../Feature_Spotlight/Secure_Private_Maps/File_Based_Replacement-kolang.md)를 참조하세요.

### 원시 교체
기본적으로(`False`) 대체 문자열은 Python의 `re.sub()`에 의해 처리됩니다. 이는 캡처된 그룹을 삽입하기 위해 `\1` 또는 `\2`와 같은 정규식 역참조 사용을 지원합니다(예: `(r'\1', r'(\d)\s+(?=\d)', 95)`).
...
* **`raw_replacement`** (부울): `True`인 경우 대체 텍스트는 순수 문자열 리터럴로 처리되고 Python의 `re.sub` 백슬래시 구문 분석을 통해 무시됩니다. 여러 줄 프롬프트나 이스케이프되지 않은 백슬래시(`\`)가 있는 문자열에 중요합니다.
*예:* `{'raw_replacement': True}`
* **`cache`** (부울): AURA 결과 캐시를 토글합니다. 동적 출력(예: 현재 시간, 무작위 농담)을 생성하는 규칙에 대해 'False'로 설정하면 매 경기마다 새로 평가됩니다.
*예:* `{'캐시': 거짓}`
