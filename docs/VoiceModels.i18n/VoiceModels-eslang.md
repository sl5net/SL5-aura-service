...
```bash
modelos de discos compactos
wget -q --show-progress https://alphacephei.com/vosk/models/vosk-model-small-de-0.15.zip
descomprimir -q vosk-model-small-de-0.15.zip
rm vosk-modelo-pequeño-de-0.15.zip
cd ..
```
...
```bash
sudo intercambio -a
sudo fallocate -l 8G /archivo de intercambio
sudo chmod 600 /archivo de intercambio
sudo mkswap /swapfile
sudo swapon /swapfile
echo '/archivo de intercambio ninguno swap sw 0 0' | sudo tee -a /etc/fstab
```
